...
/// A price reduced to its dimension's canonical unit: `$/100g` for mass, `$/each` for count.
...
nonisolated struct NormalizedPrice: Equatable, Sendable {
    ...
    /// `nil` for non-positive money/quantity, or a mass quantity that cannot reach grams.
